...
class ReVealMTTerrain:
    ...
    def __init__(self, attach_dbm: float = -87.5, seed: int = 20260830,
                 ...
                 terrain=None):
        # `terrain`: DataFrame with lat, lon, J_orth, v_orth -- ITU-R P.526
        # knife-edge loss and the Fresnel parameter over the DEM profile from
        # the serving mast, each already orthogonalised against log-distance.
        # Without it the network sees only position and has to infer the shape
        # of the ground from coordinates, which it cannot do.
        self._terr = terrain
        ...
        self.info = SimulatorInfo(
            name="reveal-mt-terrain", label="ReVeal-MT + terrain",
    ...
    def _feat(self, lat, lon):
        """Network input: normalised position, plus terrain if supplied."""
        x, y = _enu(lat, lon)
        xy = np.column_stack([x, y])
        base = (xy - self._mu) / self._sd
        if self._terr is None:
            return base
        from scipy.spatial import cKDTree
        q = np.column_stack([np.asarray(lat, float), np.asarray(lon, float)])
        _, i = self._tree.query(q)
        t = self._tvals[i]
        return np.column_stack([base, (t - self._tmu) / self._tsd])
    ...
    def fit(self, lat, lon, rsrp, out_lat=None, out_lon=None):
        ...
        if self._terr is not None:
            from scipy.spatial import cKDTree
            self._tree = cKDTree(self._terr[["lat", "lon"]].to_numpy())
            self._tvals = self._terr[["J_orth", "v_orth"]].to_numpy()
            self._tmu, self._tsd = self._tvals.mean(0), self._tvals.std(0) + 1e-9
        n_in = 2 if self._terr is None else 4
        layers, d = [], n_in
    ...
    def macro_rsrp(self, lat, lon):
        ...
        with torch.no_grad():
            v = self._total(
                torch.tensor(self._feat(lat, lon), dtype=torch.float32,
                             device=self._dev),
    ...
    def node_rsrp(self, tx_lat, tx_lon, agl_m, eirp_deficit_db, lat, lon):
        ...
        with torch.no_grad():
            Xn = torch.tensor(self._feat(lat, lon), dtype=torch.float32,
                              device=self._dev)
    ...
    def refit(self, train):
        ...
        new = ReVealMTTerrain(self.attach_dbm, self.seed, self.epochs,
                              self._device, self._terr)
